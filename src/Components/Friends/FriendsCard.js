...
export default function FriendsCard(props) {
  ...
  const getId = () => {
    return <FriendProfile key={friend.fId} />
  }
  
  return (
    <div className="friend-card-border col-3">
      <Link to={profilePath} className="f-image-display" onClick={getId}>
        <img src={friend.fImg} alt={friend.alt} className="friend-list-pics" />
      </Link>
      <Link to={profilePath} className="f-fname-display" onClick={getId}>
